...
- **Mitigation**: Vérifié empiriquement — `hnswlib-node@3.0.0` **ne fournit aucun binaire prébuild** : il publie les sources et un `binding.gyp`, et c'est le `node-gyp rebuild` implicite de npm (exécuté lors d'une installation normale, y compris pour un paquet sans hook `install` déclaré) qui génère `build/Release/addon.node`. D'où l'étape explicite `npm rebuild hnswlib-node` après `--ignore-scripts` : sans elle, le module natif ne se charge pas et les 3 suites important `MultimodalEmbeddingService` échouent (constaté : 3 suites en échec / 74 en succès). `bufferutil`/`utf-8-validate` sont des accélérateurs optionnels de `ws` (repli JS natif). Aucun test unitaire ne lance de navigateur (`BrowserService.test.ts` et `browserTools.test.ts` n'ont aucune référence à `playwright`/`chromium`/`launch(` ; `TlsImpersonator.test.ts` ne teste que le chiffrement TLS).
