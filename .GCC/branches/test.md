...
- **Re-exécution du scan d'historique (voie `pre-push`) sur le dernier commit**: `bash .githooks/_common/detect-secrets.sh history` → `15 commits scanned`, `13.61 MB`, `no leaks found`, code de sortie relevé hors pipeline = `0`. Remplace la lecture `10 commits` / `13.57 MB` consignée plus haut, qui datait d'avant les commits de journaux.
