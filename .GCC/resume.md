...
- **Rien de cassé ni de contourné.** `git push` jamais exécuté (non demandé). `git status` ne montre plus que les retouches de relecture de ces journaux, portées par leur propre commit `docs(gcc)`.
