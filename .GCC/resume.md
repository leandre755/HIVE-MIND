...
2. **Immediate Action**: la dette des chemins absolus machine — réécrire les 120 liens `file:///…` de `documentation/` et `documentations/` en chemins relatifs au dépôt, et faire arbitrer par le mainteneur les 28 cibles orphelines (déplacées vers le dépôt TUI sibling). Les journaux `.GCC/`, eux, se committent en `docs(gcc)` à la fin de la session en cours.
