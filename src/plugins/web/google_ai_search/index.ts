...
export default {
  ...
  description:
    'Fast web search via Google AI Mode (SerpApi). Synthesized, sourced, and up-to-date answers.',
  version: '1.0.0',
  enabled: true,
  ...
  toolDefinition: {
    ...
    function: {
      name: 'google_ai_search',
      description:
        'Intelligent web search via Google AI Mode. Returns a sourced synthesis. Ideal for fact-checking, news, technology monitoring, and questions requiring recent data.',
      parameters: {
        type: 'object',
        properties: {
          query: {
          ...
          mode: {
            ...
            description:
              'standard = isolated question (default), chat = continue previous conversation, new = start new thread',
          },
        },
        required: ['query'],
      },
  ...
  async execute(
    args: unknown,
    _context: GoogleAiSearchContext,
    _toolName?: string,
  ...
  formatSerpApiResponse(
    ...
    if (useConversation) {
      const newToken =
        data.search_metadata?.subsequent_request_token || data.subsequent_request_token;
      if (newToken) {
  ...
  async searchGoogleAI(query: string, mode: string): Promise<SearchResult> {
    ...
    try {
      ...
      return this.formatSerpApiResponse(data, query, useConversation);
    } catch (error: unknown) {
      clearTimeout(timeoutId);
