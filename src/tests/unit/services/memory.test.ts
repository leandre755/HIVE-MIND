...
const mockResolveContextFromLegacyId = jest.fn<(id: string) => Promise<ResolvedContext | null>>();
...
const generateTagsMock = jest.fn<(content: string) => Promise<string[]>>().mockResolvedValue(['t']);

jest.unstable_mockModule('../../../services/tagService.js', () => ({
  tagService: { generateTags: generateTagsMock },
}));
jest.unstable_mockModule('../../../services/supabase.js', () => ({
  supabase: { from: mockFrom, rpc: mockRpc },
  db: { resolveContextFromLegacyId: mockResolveContextFromLegacyId },
}));
      ...
      get(_target: object, prop: string | symbol): unknown {
        if (prop === 'then') {
          return (ok?: (v: unknown) => unknown, ko?: (e: unknown) => unknown) =>
            Promise.resolve(resolver(calls)).then(ok, ko);
        }
        return (...args: unknown[]) => {
...
const rows = (n: number) => Array.from({ length: n }, (_, i) => row(String(i), 'user', 'c'));
...
describe('semanticMemory - alignement context_id', () => {
  ...
  it('recall : rappel local puis global via GLOBAL_CONTEXT_ID', async () => {
    ...
    mockRpc.mockImplementation(async (_fn, params) =>
      params.match_context_id === 'uuid-3'
        ? { data: [row('1', 'user', 'privé')], error: null }
        : { data: [row('2', 'system', 'global')], error: null },
    );
    const result = await semanticMemory.recall('123@s.whatsapp.net', 'requête', 5);
  ...
  it('recall : le rappel global reste joignable quand la résolution de contexte échoue', async () => {
    ...
    useEmbeddings(async () => [0.1, 0.2]);
    mockRpc.mockResolvedValueOnce({ data: [row('2', 'system', 'global')], error: null });
    const result = await semanticMemory.recall('123@s.whatsapp.net', 'requête', 5);
  ...
  it('recall : survit au rejet de la promesse de rappel local et exécute le rappel global', async () => {
    ...
    mockRpc.mockResolvedValueOnce({ data: [row('3', 'system', 'global')], error: null });
    const result = await semanticMemory.recall('123@s.whatsapp.net', 'requête', 5);
    expect(result).toHaveLength(1);
  ...
  it('summarize : pas de résumé IA quand les vieux messages sont trop peu nombreux', async () => {
    const chains = installQueryMock((calls) =>
      calls.some((c) => c.method === 'limit') ? { data: rows(4) } : { count: 120 },
    );
  ...
  it('factsMemory : remember et forget utilisent context_id et le conflit (context_id, key)', async () => {
    ...
    mockResolveContextFromLegacyId.mockResolvedValue({ context_id: 'uuid-9', type: 'user' });
  ...
  it('factsMemory : getAll et get filtrent sur context_id', async () => {
    ...
    mockResolveContextFromLegacyId.mockResolvedValue({ context_id: 'uuid-10', type: 'user' });
