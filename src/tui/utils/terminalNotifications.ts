...
export function isNotificationsEnabled(settings: LoadedSettings): boolean {
  const general = settings.merged.general as { enableNotifications?: boolean } | undefined;
...
export function getNotificationMethod(settings: LoadedSettings): TerminalNotificationMethod {
...
function wrapWithPassthrough(sequence: string): string {
  ...
  if (capabilityManager.isTmux()) {
    const escapedSequence = sequence.split(ESC).join(ESC + ESC);
    return ESC + 'Ptmux;' + escapedSequence + ESC + '\\';
