...
function formatLongRelative(diffDays: number, diffHours: number, diffMinutes: number): string {
  if (diffDays > 0) {
...
export const formatRelativeTime = (timestamp: string, style: 'long' | 'short' = 'long'): string => {
  ...
  if (style === 'short') {
    return formatShortRelative(diffSeconds, diffMinutes, diffHours, diffDays);
  }
  return formatLongRelative(diffDays, diffHours, diffMinutes);
};
...
async function loadSessionFileEntry(
  file: string,
  chatsDir: string,
  ...
  try {
    ...
    const fileTimestamp =
      !content.startTime || !content.lastUpdated
        ? (await safeStat(filePath).catch(() => undefined))?.mtime.toISOString()
        : undefined;
...
export const getAllSessionFiles = async (
  ...
  try {
    const files = (await safeReaddir(chatsDir)) as string[];
    ...
    const sessionPromises = sessionFiles.map((file) =>
      loadSessionFileEntry(file, chatsDir, currentSessionId, options),
    );
...
export class SessionSelector {
  ...
  constructor(projectRootOrChatsDir?: string) {
    if (
      projectRootOrChatsDir &&
      (projectRootOrChatsDir.includes('/') || projectRootOrChatsDir.includes('\\'))
    ) {
  ...
  async sessionExists(id: string): Promise<boolean> {
    const files = (await safeReaddir(this.chatsDir).catch(() => [])) as string[];
  ...
  async findSession(identifier: string): Promise<SessionInfo> {
    ...
    const sessionByUuid = sortedSessions.find((session) => session.id === trimmedIdentifier);
    if (sessionByUuid) {
...
function messageTypeToHistoryType(msg: MessageRecord): MessageType {
  switch (msg.type) {
    case 'user':
...
function pushThoughts(uiHistory: HistoryItemWithoutId[], msg: MessageRecord): void {
...
function pushToolCalls(uiHistory: HistoryItemWithoutId[], msg: MessageRecord): void {
...
export function convertSessionToHistoryFormats(messages: ConversationRecord['messages']): {
  ...
  for (const msg of messages) {
    ...
    if (trimmedText) {
      uiHistory.push({
        type: messageTypeToHistoryType(msg),
        text: uiText,
      });
    }

    // Add tool calls if present
    pushToolCalls(uiHistory, msg);
  }
