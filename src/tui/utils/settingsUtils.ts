...
function flattenSchema(schema: SettingsSchema, prefix = ''): FlattenedSchema {
  ...
  const resultMap = new Map<string, SettingDefinition & { key: string }>();
  for (const [key, definition] of schemaMap) {
    const newKey = prefix ? `${prefix}.${key}` : key;
    resultMap.set(newKey, { ...definition, key: newKey });
    if (definition.properties) {
      const nested = flattenSchema(definition.properties, newKey);
      for (const [nestedKey, nestedDef] of new Map(Object.entries(nested))) {
        resultMap.set(nestedKey, nestedDef);
      }
    }
  }
  result = Object.fromEntries(resultMap);
  return result;
}
...
> {
  const categories = new Map<string, Array<SettingDefinition & { key: string }>>();
...
export function getSettingDefinition(
  ...
  return new Map<string, SettingDefinition & { key: string }>(
    Object.entries(getFlattenedSchema()),
  ).get(key);
}

export function requiresRestart(key: string): boolean {
  return getSettingDefinition(key)?.requiresRestart ?? false;
}

export function getDefaultValue(key: string): SettingsValue {
  return getSettingDefinition(key)?.default as SettingsValue;
}
 ...
 */
export function getEffectiveDefaultValue(key: string, config?: HiveConfig): SettingsValue {
  if (key === 'model.compressionThreshold' && config) {
    const experiments = config.getExperiments();
    const flagEntry = new Map<string, unknown>(Object.entries(experiments?.flags ?? {})).get(
      ExperimentFlags.CONTEXT_COMPRESSION_THRESHOLD,
    ) as { floatValue?: number } | undefined;
    const experimentValue = flagEntry?.floatValue;
...
export function getDialogRestartRequiredSettings(): string[] {
  return Object.values(getFlattenedSchema())
    .filter((definition) => definition.requiresRestart && definition.showInDialog !== false)
    .map((definition) => definition.key);
}
...
export function getEffectiveValue(key: string, settings: Settings): SettingsValue {
  ...
  return definition.default as SettingsValue;
}
...
export function getSettingCategory(key: string): string | undefined {
  return getSettingDefinition(key)?.category;
}

export function shouldShowInDialog(key: string): boolean {
  return getSettingDefinition(key)?.showInDialog ?? true; // Default to true for backward compatibility
}
...
export function parseEditedValue(type: SettingsType, newValue: string): SettingsValue | null {
  if (type === 'number') {
    if (newValue.trim() === '') {
      return null;
    }
