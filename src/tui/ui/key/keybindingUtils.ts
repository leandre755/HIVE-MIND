...
export function formatKeyBinding(binding: KeyBinding, platform?: string): string {
  const activePlatform =
    platform ?? (process.env['FORCE_GENERIC_KEYBINDING_HINTS'] ? 'default' : process.platform);
  const modMap =
    (Reflect.get(MODIFIER_MAPS, activePlatform) as typeof MODIFIER_MAPS.default | undefined) ||
    MODIFIER_MAPS.default;
  const parts: string[] = [];
