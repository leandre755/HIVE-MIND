  ...
  constructor(pattern: string) {
    ...
    do {
      ...
      if (lowerRemains.startsWith('ctrl+')) {
        ...
        matched = true;
      } else if (lowerRemains.startsWith('alt+') || lowerRemains.startsWith('opt+')) {
        alt = true;
...
export const defaultKeyBindingConfig: KeyBindingConfig = new Map([
  ...
  [
    ...
    [new KeyBinding('ctrl+right'), new KeyBinding('alt+right'), new KeyBinding('alt+f')],
  ],
  ...
  [
    ...
    [new KeyBinding('ctrl+backspace'), new KeyBinding('alt+backspace'), new KeyBinding('ctrl+w')],
  ],
  [
    Command.DELETE_WORD_FORWARD,
    [new KeyBinding('ctrl+delete'), new KeyBinding('alt+delete'), new KeyBinding('alt+d')],
  ],
  ...
  [Command.OPEN_EXTERNAL_EDITOR, [new KeyBinding('ctrl+g'), new KeyBinding('ctrl+shift+g')]],
...
export const commandDescriptions: Readonly<Record<Command, string>> = {
  ...
  [Command.DEPRECATED_OPEN_EXTERNAL_EDITOR]: 'Deprecated command to open external editor.',
  [Command.PASTE_CLIPBOARD]: 'Paste from the clipboard.',

  // App Controls
  [Command.SHOW_ERROR_DETAILS]: 'Toggle the debug console for detailed error information.',
  ...
  [Command.FOCUS_SHELL_INPUT]: 'Move focus from agent to the active shell.',
  [Command.UNFOCUS_SHELL_INPUT]: 'Move focus from the shell back to agent.',
  ...
  [Command.UNFOCUS_BACKGROUND_SHELL]: 'Move focus from background shell to agent.',
  [Command.UNFOCUS_BACKGROUND_SHELL_LIST]: 'Move focus from background shell list to agent.',
  [Command.SHOW_BACKGROUND_SHELL_UNFOCUS_WARNING]:
 ...
 */
function processKeybindingItem(
  configMap: KeyBindingConfig,
  commandEntry: { command: Command; negate: boolean },
  key: string,
  errors: string[],
) {
  const { command, negate } = commandEntry;
  const currentBindings = configMap.get(command) ?? [];

  try {
    const keyBinding = new KeyBinding(key);

    if (!negate) {
      configMap.set(command, [keyBinding, ...currentBindings]);
      return;
    }
    const updatedBindings = currentBindings.filter((b) => !b.equals(keyBinding));
    if (updatedBindings.length === currentBindings.length) {
      throw new Error(`cannot remove "${key}" since it is not bound`);
    }
    configMap.set(command, updatedBindings);
  } catch (e) {
...
export async function loadCustomKeybindings(): Promise<{
  ...
  try {
    const content = await safeReadFile(userKeybindingsPath, 'utf-8');
    ...
    if (result.success) {
      ...
      for (const { commandEntry, key } of result.data) {
        processKeybindingItem(config, commandEntry, key, errors);
      }
    } else {
      errors.push(
