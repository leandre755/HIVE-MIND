...
export const RewindConfirmation: React.FC<RewindConfirmationProps> = ({
  ...
  const options = useMemo(() => {
    ...
    return REWIND_OPTIONS.filter(
      (option) =>
        option.value !== RewindOutcome.RewindAndRevert && option.value !== RewindOutcome.RevertOnly,
    );
  }, [stats]);
      ...
      <Box flexDirection="column" width={terminalWidth}>
        ...
        {stats && (
          <Box flexDirection="column">
            ...
            <Text>Note: Rewinding does not affect files edited manually or by the shell tool.</Text>
          </Box>
        )}
