...
export const ToolConfirmationQueue: React.FC<ToolConfirmationQueueProps> = ({ confirmingTool }) => {
  ...
  if (isShell || isEdit) {
    ...
    const availableContentHeight = constrainHeight ? Math.max(maxHeight - 3, 4) : undefined;

    const toolLabel = getConfirmationLabel(
      tool.name,
      tool.confirmationDetails as unknown as SerializableConfirmationDetails,
    );
  ...
  let availableContentHeight: number | undefined;
  if (constrainHeight) {
    const deduct = hideToolIdentity ? 4 : 6;
    availableContentHeight = Math.max(maxHeight - deduct, 4);
  }
    ...
    <Box flexDirection="column" width={mainAreaWidth} flexShrink={0}>
      ...
      >
        <Box flexDirection="column" width={mainAreaWidth - 4}>
          ...
          {!hideToolIdentity && (
            <Box>
              <ToolStatusIndicator status={tool.status as CoreToolCallStatus} name={tool.name} />
              <ToolInfo
                name={tool.name}
                status={tool.status as CoreToolCallStatus}
