...
const renderTabsFn = (
  ...
  const availableWidth =
    width - TAB_DISPLAY_HORIZONTAL_PADDING - getCachedStringWidth(helpTextStr) - pidInfoWidth;
  let currentWidth = 0;
  const tabs = [];
  for (let i = 0; i < shellList.length; i++) {
    const shell = shellList.at(i);
    if (!shell) continue;
    ...
    <Box flexDirection="column" height="100%" width="100%">
      ...
      <Box flexGrow={1} width="100%">
        <RadioButtonSelect
          ...
          isFocused={isFocused}
          maxItemsToShow={Math.max(1, height - TOTAL_OVERHEAD_HEIGHT - PROCESS_LIST_HEADER_HEIGHT)}
          renderItem={(item) => {
...
export const BackgroundTaskDisplay = ({
  ...
  const { dismissBackgroundTask, setActiveBackgroundTaskPid, setIsBackgroundTaskListOpen } =
    useUIActions();
  ...
  useEffect(() => {
    ...
    const unsubscribe = ShellExecutionService.subscribe(activePid, (event: unknown) => {
      const evt = event as { type: string; chunk?: unknown };
      if (evt.type === 'data') {
        if (typeof evt.chunk === 'string') {
          if (!subscribedRef.current) {
            ...
            setOutput((prev) =>
              typeof prev === 'string' ? prev + evt.chunk : (evt.chunk as AnsiOutput),
            );
          }
        } else {
          // PTY always sends full AnsiOutput
          setOutput(evt.chunk as AnsiOutput);
        }
      }
    });
  ...
  useKeypress(
    (key) => {
      ...
      if (isListOpenProp) {
        return handleBgListKeys(
          key,
          highlightedPid ?? undefined,
          setIsBackgroundTaskListOpen,
          dismissBackgroundTask,
          setActiveBackgroundTaskPid,
        );
      }
      return handleBgActiveKeys(
        key,
        activeShell.pid,
        setIsBackgroundTaskListOpen,
        dismissBackgroundTask,
  ...
  const renderTabs = () =>
    renderTabsFn(
      ...
      isListOpenProp,
      helpTextStr,
      formatCommand(Command.TOGGLE_BACKGROUND_SHELL_LIST),
    );
...
function handleBgListKeys(
  key: Key,
  highlightedPid: number | undefined,
  setIsBackgroundTaskListOpen: (open: boolean) => void,
  dismissBackgroundTask: (pid: number) => Promise<void>,
  setActiveBackgroundTaskPid: (pid: number) => void,
): boolean {
  const matchers = keyMatchers as Record<string, ((k: Key) => boolean) | undefined>;
  if (matchers[Command.BACKGROUND_SHELL_ESCAPE]?.(key)) {
    setIsBackgroundTaskListOpen(false);
    return true;
  }
  if (matchers[Command.KILL_BACKGROUND_SHELL]?.(key)) {
    if (highlightedPid) void dismissBackgroundTask(highlightedPid);
    return true;
  }
  if (matchers[Command.TOGGLE_BACKGROUND_SHELL_LIST]?.(key)) {
    if (highlightedPid) setActiveBackgroundTaskPid(highlightedPid);
    setIsBackgroundTaskListOpen(false);
    return true;
  }
  return false;
...
function handleBgActiveKeys(
  ...
  const matchers = keyMatchers as Record<string, ((k: Key) => boolean) | undefined>;
  if (matchers[Command.TOGGLE_BACKGROUND_SHELL]?.(key)) return false;

  if (matchers[Command.KILL_BACKGROUND_SHELL]?.(key)) {
    void dismissBackgroundTask(activePid);
    return true;
  }
  ...
  if (matchers[Command.BACKGROUND_SHELL_SELECT]?.(key)) {
    ShellExecutionService.writeToPty(activePid, '\r');
    return true;
  }
  if (matchers[Command.DELETE_CHAR_LEFT]?.(key)) {
    ShellExecutionService.writeToPty(activePid, '\b');
    return true;
  }
  if (key.sequence) {
    ShellExecutionService.writeToPty(activePid, key.sequence);
    return true;
  }
  return false;
}
