...
const ModelUsageTable: React.FC<ModelUsageTableProps> = ({ models }) => {
    ...
    if (metrics.roles) {
      ...
      roleEntries.forEach(([role, roleMetrics]) => {
        rows.push({
          ...
          cachedTokens: (roleMetrics.tokens?.cached ?? 0).toLocaleString(),
          inputTokens: (roleMetrics.tokens?.prompt ?? 0).toLocaleString(),
          outputTokens: (roleMetrics.tokens?.candidates ?? 0).toLocaleString(),
          isSubRow: true,
        });
      });
    }
  ...
  return (
    <Box flexDirection="column" marginTop={1}>
      ...
      <Text color={theme.text.secondary}>Use /model to view model quota information</Text>
      <Box height={1} />
...
export const StatsDisplay: React.FC<StatsDisplayProps> = ({ duration, title, footer }) => {
  const { stats } = useSessionStats();
  const { metrics } = stats;
  const { files, models } = metrics;
  const tools = metrics.tools ?? { totalCalls: 0, totalSuccess: 0, totalFail: 0, byName: {} };
  ...
  return (
    ...
    >
      ...
      <Section title="Interaction Summary">
        ...
        {showUserIdentity && (
          <StatRow title="AI Providers:">
            <Text color={theme.text.primary}>
              {`${providerStatus.activeFamilies}/${providerStatus.totalFamilies} active (${providerStatus.totalKeys} keys)`}
            </Text>
          </StatRow>
        )}
      ...
      {Object.keys(models).length > 0 && (
        <ModelUsageTable models={models as unknown as Record<string, ModelMetrics>} />
      )}
