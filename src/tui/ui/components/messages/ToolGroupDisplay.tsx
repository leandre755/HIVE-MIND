...
function renderToolsContent(
  ...
  otherTools: ToolDisplayItem[],
  noticeToolsCount: number,
  borderTop: boolean | undefined,
  borderBottom: boolean | undefined,
  borderColor: string | undefined,
  borderDimColor: boolean | undefined,
  isToolGroupBoundary: boolean | undefined,
): React.ReactNode {
  if (shouldShowBox) {
    const boxBorderTop = borderTop || noticeToolsCount > 0;
    return (
      <Box
  ...
  if (otherTools.length > 0) {
    return (
      <Box
        flexDirection="column"
        marginTop={noticeToolsCount > 0 ? 1 : 0}
        marginBottom={borderBottom ? 1 : 0}
      >
        {otherTools.map((tool, index) => (
          <ToolDisplayMessage key={`tool-${index}`} tool={tool} />
...
export const ToolGroupDisplay: React.FC<ToolGroupDisplayProps> = ({
  ...
  const visibleTools = tools.filter((t) => t.status !== CoreToolCallStatus.AwaitingApproval);
  const noticeTools = visibleTools.filter((t) => t.format === 'notice');
  const otherTools = visibleTools.filter((t) => t.format !== 'notice' && t.format !== 'hidden');
  ...
  const shouldShowBox = Boolean((hasOtherTools || isClosingSlice) && !isCompactModeEnabled);
    ...
    <Box flexDirection="column">
      {noticeTools.map((tool, index) => {
        ...
        return (
          <Box
            key={`notice-${index}`}
            marginTop={isFirstInGroup || index > 0 ? 1 : 0}
...
const ToolDisplayMessage: React.FC<ToolDisplayMessageProps> = ({ tool }) => {
  ...
  if (!tool.name && !tool.description && !tool.result && !tool.resultSummary) {
    return (
      <Box paddingLeft={2}>
        <ToolStatusIndicator
          status={tool.status ?? CoreToolCallStatus.Pending}
  ...
  const {
    status = CoreToolCallStatus.Pending,
  ...
  if (isCompact) {
    return (
      <Box paddingLeft={2} flexDirection="row" flexWrap="wrap">
        <ToolStatusIndicator status={status} name={name || tool.originalRequestName || ''} />
...
const ToolResultDisplayContent: React.FC<ToolResultDisplayContentProps> = ({
  ...
  const contentObj = content as ToolResultContent;
  switch (contentObj.type) {
    case 'text':
      return (
        <Box flexDirection="column">
          <Text color={theme.text.primary}>{contentObj.text}</Text>
          {summary && (
            <Box marginTop={0}>
      ...
      return (
        <Box flexDirection="column">
          ...
          <Text color={theme.text.secondary}>
            {`[Diff Display: ${contentObj.beforeText?.length ?? 0} -> ${contentObj.afterText?.length ?? 0} chars]`}
          </Text>
        </Box>
      );
    case 'terminal':
    ...
    case 'agent':
      return <Text color={theme.text.secondary}>[Subagent: {contentObj.threadId}]</Text>;
    default:
      return null;
  }
};
