...
function getResetKey(resultDisplay: unknown): number | boolean {
  if (typeof resultDisplay === 'string' || Array.isArray(resultDisplay)) {
    return resultDisplay.length;
  }
  return Boolean(resultDisplay);
}
...
export function useFocusHint(
  ...
  const resetKey = getResetKey(resultDisplay);

  const showFocusHint = useInactivityTimer(shellFocusable, resetKey, SHELL_FOCUS_HINT_DELAY_MS);
  ...
  const shouldShowFocusHint = shellFocusable && (showFocusHint || userHasFocused);
...
function getToolStatusColor(isFocused: boolean | undefined, isShell: boolean): string {
  if (isFocused) return theme.ui.focus;
  if (isShell) return theme.ui.active;
...
export const ToolStatusIndicator: React.FC<ToolStatusIndicatorProps> = ({
  ...
  const statusColor = getToolStatusColor(isFocused, isShell);
...
export const ToolInfo: React.FC<ToolInfoProps> = ({
  ...
  return (
    <Box overflow="hidden" height={isExpanded ? undefined : 1} flexGrow={1} flexShrink={1}>
      <Text
