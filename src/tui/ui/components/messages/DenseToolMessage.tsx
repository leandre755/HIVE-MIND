...
const hasPayload = (res: unknown): res is PayloadResult => {
  ...
  if (typeof res !== 'object' || res === null || !('payload' in res)) return false;
...
function getFileOpData(
  ...
  const removed =
    (diff.diffStat?.model_removed_lines ?? 0) + (diff.diffStat?.user_removed_lines ?? 0);
...
function getReadManyFilesData(result: ReadManyFilesResult): ViewParts {
  const resultTyped = result as ReadManyFilesResult & { include?: string[]; skipped?: unknown[] };
  const includePatterns = resultTyped.include?.join(', ') ?? '';
  const description = (
    <Text color={theme.text.secondary} wrap="truncate-end">
  ...
  const skippedCount = resultTyped.skipped?.length ?? 0;
...
function getListDirectoryData(
  ...
  const summaryText =
    ...
    `Listed ${result.entries.length} entry(ies) in ${result.path}`;
  const summary = <Text color={theme.text.accent}>→ {summaryText}</Text>;

  return { description, summary, payload: undefined };
}

function getListResultData(result: unknown, originalDescription?: string): ViewParts {
  if (isReadManyFilesResult(result)) {
    return getReadManyFilesData(result as ReadManyFilesResult);
  }
  if (isListResult(result)) {
    return getListDirectoryData(result as ListDirectoryResult, originalDescription);
  }
  return getGenericSuccessData(result, originalDescription);
}

function getGenericSuccessData(resultDisplay: unknown, originalDescription?: string): ViewParts {
  let summary: React.ReactNode;
  let payload: React.ReactNode;
  const display = resultDisplay as { summary?: string; payload?: string };
  ...
  if (typeof resultDisplay === 'string') {
    ...
    summary = <Text color={theme.text.accent}>→ {display.summary ?? ''}</Text>;
    payload = (
      <Box marginLeft={2}>
        <Text color={theme.text.secondary}>{display.payload ?? ''}</Text>
      </Box>
    );
  } else {
    summary = (
...
interface DenseToolMessageViewProps {
  ...
  terminalWidth: number;
  isFocused: boolean;
  toggleRef: React.RefObject<DOMElement | null>;
}

function DenseToolMessageView({
  name,
  status,
  description,
  summary,
...
export const DenseToolMessage: React.FC<DenseToolMessageProps> = (props) => {
  ...
  const isExpanded = isExpandedInContext ? isExpandedInContext(callId) : localIsExpanded;

  const toggleRef = useRef<DOMElement | null>(null);
  ...
  const diff = useMemo((): FileDiff | undefined => {
    if (isFileDiff(resultDisplay)) return resultDisplay as unknown as FileDiff;
    if (confirmationDetails?.type === 'edit') {
      const details = confirmationDetails as unknown as Extract<
        SerializableConfirmationDetails,
        { type: 'edit' }
      >;
      return {
        ...
        oldContent: '',
        newContent: '',
        diffStat: (details as unknown as { diffStat?: FileDiff['diffStat'] }).diffStat,
      };
    }
    return undefined;
  ...
  }, [isAlternateBuffer, diff, isExpanded, diffLines.length, viewParts.payload, outputFile]);

  return (
    <DenseToolMessageView
      name={name}
      status={status}
