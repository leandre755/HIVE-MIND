...
const StatusDot: React.FC<{ ok: boolean }> = ({ ok }) => (
  <Text color={ok ? theme.status.success : theme.status.error}>{ok ? '\u25CF' : '\u25CB'}</Text>
);
    ...
    <Box>
      ...
      <Text>
        ...
        {serverRunning ? (
          <Text>
            port {serverPort}
            {serverPid ? <Text color={theme.text.secondary}> (PID {serverPid})</Text> : null}
          </Text>
        ) : (
          <Text color={theme.status.error}>not running on port {serverPort}</Text>
        )}
      </Text>
    </Box>
    ...
    <Box marginTop={1}>
      {allPassing ? (
        <Box flexDirection="column">
          ...
          <Text color={theme.text.secondary}>This happens automatically on every request.</Text>
        </Box>
      ) : (
