 ...
 */
function formatHiddenLinesMessage(
  count: number,
  isNarrow: boolean,
  showMoreKey: string,
  ...
  if (isNarrow) {
    return `... ${count} hidden (${showMoreKey}) ...`;
  }
  const plural = count === 1 ? '' : 's';
  const pos = direction === 'top' ? 'first' : 'last';
  return `... ${pos} ${count} line${plural} hidden (${showMoreKey} to show) ...`;
}
  ...
  return (
    <Box flexDirection="column" width={maxWidth} maxHeight={effectiveMaxHeight} flexShrink={0}>
      {totalHiddenLines > 0 && overflowDirection === 'top' && (
        <Box paddingX={paddingX}>
          <Text color={theme.text.secondary} wrap="truncate">
            {formatHiddenLinesMessage(totalHiddenLines, isNarrow, showMoreKey, 'top')}
          </Text>
        </Box>
      )}
      <Box
      ...
      {totalHiddenLines > 0 && overflowDirection === 'bottom' && (
        <Box paddingX={paddingX}>
          <Text color={theme.text.secondary} wrap="truncate">
            {formatHiddenLinesMessage(totalHiddenLines, isNarrow, showMoreKey, 'bottom')}
          </Text>
        </Box>
      )}
    </Box>
