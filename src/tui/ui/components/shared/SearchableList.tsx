...
export interface SearchableListProps<T extends GenericListItem> {
  ...
  useSearch: (props: { items: T[]; onSearch?: (query: string) => void }) => SearchListState<T>;
...
export function SearchableList<T extends GenericListItem>({
  ...
  return (
    <Box flexDirection="column" width="100%" height="100%" paddingX={1}>
      ...
      {searchBuffer && (
        <Box borderStyle="round" borderColor={theme.border.default} paddingX={1} marginBottom={1}>
          <TextInput buffer={searchBuffer} placeholder={searchPlaceholder} focus={isFocused} />
        </Box>
      )}
