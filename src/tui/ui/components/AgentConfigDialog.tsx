...
const AGENT_CONFIG_FIELDS: AgentConfigField[] = [
  ...
  },
  {
    key: 'model',
    label: 'Model',
    description: "Model to use (e.g., 'gpt-4o', 'claude-3-5-sonnet', or 'inherit')",
    type: 'string',
...
export function AgentConfigDialog({
  ...
  const [pendingOverride, setPendingOverride] = useState<AgentOverride>(() => {
    ...
    const agentsConfig = scopeSettings.agents as
      Record<string, Record<string, unknown>> | undefined;
    const overrides = agentsConfig?.overrides;
    const existingOverride = (
      isRecord(overrides) ? Reflect.get(overrides, agentName) : undefined
    ) as AgentOverride | undefined;
  ...
  const handleItemToggle = useCallback(
    (key: string, _item: SettingsDialogItem) => {
      ...
      const newOverride = setNestedValue(pendingOverride, field.path, newValue) as AgentOverride;
  ...
  const handleEditCommit = useCallback(
    (key: string, newValue: string, _item: SettingsDialogItem) => {
      ...
      const newOverride = setNestedValue(pendingOverride, field.path, parsed) as AgentOverride;
  ...
  const handleItemClear = useCallback(
    (key: string, _item: SettingsDialogItem) => {
      ...
      const newOverride = setNestedValue(pendingOverride, field.path, undefined) as AgentOverride;
    ...
    <BaseSettingsDialog
      ...
      footer={
        modifiedFields.size > 0
          ? {
              content: <Text color={theme.text.secondary}>Changes saved automatically.</Text>,
              height: 1,
            }
          : undefined
      }
