...
function footerConfigReducer(
  ...
  switch (action.type) {
    case 'MOVE_ITEM': {
      ...
      const valCurrent = newOrderedIds.at(currentIndex);
      const valNew = newOrderedIds.at(newIndex);
      if (valCurrent !== undefined && valNew !== undefined) {
        Reflect.set(newOrderedIds, currentIndex, valNew);
        Reflect.set(newOrderedIds, newIndex, valCurrent);
      }
...
const buildPreviewContent = (
  ...
  const rowItems: FooterRowItem[] = itemsToPreview
    .filter((id: string) => Boolean(Reflect.get(mockData, id)))
    .map((id: string) => ({
      key: id,
      header: ALL_ITEMS.find((i) => i.id === id)?.header ?? id,
      element: Reflect.get(mockData, id) as React.ReactNode,
      flexGrow: 0,
...
const makeRenderFooterConfigItem =
  (selectedIds: Set<string>, showLabels: boolean) =>
  (
    item: SelectionListItem<FooterConfigItem>,
    { isSelected, titleColor }: { isSelected: boolean; titleColor: string },
  ) => {
    const configItem = item.value;
    let isChecked = false;
    if (configItem.type === 'config') {
      isChecked = selectedIds.has(configItem.id);
    } else if (configItem.type === 'labels-toggle') {
    ...
    return (
      <Box flexDirection="column" minHeight={2}>
        <Box flexDirection="row">
          ...
          <Text
            color={configItem.type === 'reset' && isSelected ? theme.status.warning : titleColor}
          >
            {configItem.type !== 'reset' ? ' ' : ''}
            {configItem.label}
          </Text>
...
export const FooterConfigDialog: React.FC<FooterConfigDialogProps> = ({ onClose }) => {
  ...
  const handleSaveAndClose = useCallback(() => {
    ...
    const effectiveCurrent = currentSetting ?? deriveItemsFromLegacySettings(settings.merged);
  ...
  return (
    ...
    >
      ...
      <Text color={theme.text.secondary}>Select which items to display in the footer.</Text>
