    ...
    <Box flexDirection="column" flexShrink={0} flexGrow={0} width={uiState.terminalWidth}>
      ...
      {uiState.pendingHistoryItems.map((item, i) => (
        <HistoryItemDisplay
          ...
          availableTerminalHeight={undefined}
          terminalWidth={uiState.mainAreaWidth}
          item={{ ...item, id: 0 } as unknown as Parameters<typeof HistoryItemDisplay>[0]['item']}
          isPending={true}
        />
      ))}
      {showPromptedTool && (
        <Box flexDirection="column" marginTop={1} marginBottom={1}>
          ...
          <Box marginTop={1}>
            <ToolStatusIndicator
              status={
                confirmingTool.tool.status as unknown as Parameters<
                  typeof ToolStatusIndicator
                >[0]['status']
              }
            ...
            <ToolInfo
              name={confirmingTool.tool.name}
              status={
                confirmingTool.tool.status as unknown as Parameters<typeof ToolInfo>[0]['status']
              }
