...
export const ColorsDisplay: React.FC<ColorsDisplayProps> = ({ activeTheme }) => {
   ...
   */
  function addFlattenedRow(
    newPath: string,
    value: string,
    bgRows: BackgroundColorRow[],
    stdRows: StandardColorRow[],
  ) {
    if (newPath.startsWith('background.')) {
      bgRows.push({ type: 'background', name: newPath, value });
    } else {
      stdRows.push({ type: 'standard', name: newPath, value });
    }
  }

  const flattenColors = (obj: object, path: string = '') => {
    for (const [key, value] of Object.entries(obj)) {
      if (!value || (key === 'gradient' && Array.isArray(value))) continue;
      const newPath = path ? `${path}.${key}` : key;

      if (typeof value === 'object' && !Array.isArray(value)) {
        flattenColors(value, newPath);
      } else if (typeof value === 'string') {
        addFlattenedRow(newPath, value, backgroundRows, standardRows);
      }
    }
  };
      ...
      <Box marginBottom={1} flexDirection="column">
        ...
        <Box marginTop={1} flexDirection="column">
          ...
          <Box marginLeft={2} flexDirection="column">
            <Text color={theme.text.primary}>
              • <Text bold>Hex:</Text> Rendered exactly by modern terminals. Not overridden by app
              themes.
            </Text>
            <Text color={theme.text.primary}>
              • <Text bold>Blank:</Text> Uses your terminal&apos;s default foreground/background.
            </Text>
            <Text color={theme.text.primary}>
              • <Text bold>Compatibility:</Text> On older terminals, hex is approximated to the
              nearest ANSI color.
            </Text>
            <Text color={theme.text.primary}>
              • <Text bold>ANSI Names:</Text> &apos;red&apos;, &apos;green&apos;, etc. are mapped to
              your terminal app&apos;s palette.
            </Text>
          </Box>
        </Box>
      </Box>
...
function renderStandardRow({ name, value }: StandardColorRow) {
  ...
  const description = (Reflect.get(COLOR_DESCRIPTIONS, name) as string | undefined) || '';
...
function renderBackgroundRow({ name, value }: BackgroundColorRow) {
  ...
  return (
    <Box key={name} flexDirection="row" paddingX={1}>
      <Box width={VALUE_COLUMN_WIDTH} backgroundColor={value} justifyContent="center" paddingX={1}>
