  ...
  const flashScrollbar = useCallback(() => {
    ...
    const startFadeOut = () => {
      ...
      const animateFadeOut = () => {
        ...
        const animElapsed = Date.now() - fadeOutStart;
        const animProgress = Math.max(0, Math.min(animElapsed / fadeOutDuration, 1));
        setScrollbarColor(interpolateColor(focusedColor, unfocusedColor, animProgress));
    ...
    const animateFadeIn = () => {
      ...
      if (progress === 1) {
        ...
        // Phase 2: Wait & Phase 3: Fade Out
        timeout.current = setTimeout(startFadeOut, visibleDuration);
      }
    };
