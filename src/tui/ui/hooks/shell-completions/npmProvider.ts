...
const NPM_SUBCOMMANDS = ['build', 'ci', 'dev', 'install', 'publish', 'run', 'start', 'test'];
...
export const npmProvider: ShellCompletionProvider = {
  ...
  async getCompletions(
    ...
    if (cursorIndex === 1) {
      ...
      return {
        suggestions: NPM_SUBCOMMANDS.filter((cmd) => cmd.startsWith(partial)).map((cmd) => ({
          label: cmd,
          value: cmd,
    ...
    if (cursorIndex === 2 && tokens[1] === 'run') {
      ...
      try {
        ...
        const content = await safeReadFile(pkgJsonPath, 'utf8');
