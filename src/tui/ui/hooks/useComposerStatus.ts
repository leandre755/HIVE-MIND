...
export const useComposerStatus = () => {
  ...
  const hasPendingToolConfirmation = useMemo(
    () =>
      (uiState.pendingHistoryItems ?? [])
        ...
        .some((item) =>
          item.tools.some((tool) => tool.status === CoreToolCallStatus.AwaitingApproval),
        ),
    [uiState.pendingHistoryItems],
  );
  ...
  const modeContentObj = useMemo(() => {
    const hideMinimalModeHintWhileBusy =
      !uiState.cleanUiDetailsVisible && (showLoadingIndicator || uiState.activeHooks.length > 0);
   ...
   */
  const shouldCollapseDuringApproval = settings.merged.ui.collapseDrawerDuringApproval !== false;
