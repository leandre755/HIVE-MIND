...
export function useSearchBuffer({ initialText = '', onChange }: UseSearchBufferProps): TextBuffer {
  const { mainAreaWidth } = useUIState();
  const viewportWidth = Math.max(MIN_VIEWPORT_WIDTH, mainAreaWidth - VIEWPORT_WIDTH_OFFSET);
