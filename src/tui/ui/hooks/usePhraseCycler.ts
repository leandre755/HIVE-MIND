...
export const usePhraseCycler = (
  ...
  const [currentWittyPhraseState, setCurrentWittyPhraseState] = useState<string | undefined>(
    undefined,
  );
  ...
  useEffect(() => {
    ...
    const setRandomTip = (force: boolean = false) => {
      if (!showTips) {
        if (lastSelectedTipRef.current !== undefined) {
      ...
      if (!force && lastSelectedTipRef.current !== undefined) {
        return;
      }
      ...
      if (filteredTips.length > 0) {
        const selected = filteredTips[randomInt(0, filteredTips.length)];
    ...
    const setRandomWitty = (force: boolean = false) => {
      if (!showWit) {
        if (lastSelectedWittyPhraseRef.current !== undefined) {
      ...
      if (!force && lastSelectedWittyPhraseRef.current !== undefined) {
        return;
      }
      ...
      if (filteredWitty.length > 0) {
        const selected = filteredWitty[randomInt(0, filteredWitty.length)];
    ...
    return clearTimers;
  }, [isActive, isWaiting, shouldShowFocusHint, showTips, showWit, customPhrases, maxLength]);
