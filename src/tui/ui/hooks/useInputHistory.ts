...
export function useInputHistory({
  ...
  const historyCacheRef = useRef<Map<number, { text: string; offset: number }>>(new Map());
  ...
  const navigateTo = useCallback(
    (nextIndex: number, defaultCursor: 'start' | 'end') => {
      ...
      historyCacheRef.current.set(prevIndexBeforeMove, {
        text: currentQuery,
      ...
      const saved = historyCacheRef.current.get(nextIndex);
      ...
      if (isReturningToPrevious && saved && saved.offset > 0 && saved.offset < cpLen(saved.text)) {
        ...
        if (saved) {
          ...
          const newValue = userMessages.at(userMessages.length - 1 - nextIndex) ?? '';
