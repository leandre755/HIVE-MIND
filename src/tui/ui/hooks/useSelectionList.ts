...
const findNextValidIndex = (
  ...
  for (let i = 0; i < len; i++) {
    const candidate = calculateNextIndex(nextIndex, step, len, wrapAround);
    if (candidate === null) {
      return currentIndex;
    }
    nextIndex = candidate;

    if (!items.at(nextIndex)?.disabled) {
      return nextIndex;
    }

    if (
      !wrapAround &&
      ((direction === 'down' && nextIndex === len - 1) || (direction === 'up' && nextIndex === 0))
    ) {
      return currentIndex;
    }
  }
...
const computeInitialIndex = (
  ...
  if (initialKey !== undefined) {
    for (const [i, item] of items.entries()) {
      if (item.key === initialKey && !item.disabled) {
        return i;
      }
    }
  }
  ...
  if (items.at(targetIndex)?.disabled) {
...
function selectionListReducer(
  ...
  switch (action.type) {
    ...
    case 'INITIALIZE': {
      ...
      const activeKey =
        initialIndex === state.initialIndex ? state.items.at(state.activeIndex)?.key : undefined;
...
function areBaseItemsEqual(a: BaseSelectionItem[], b: BaseSelectionItem[]): boolean {
  ...
  for (const [i, itemA] of a.entries()) {
    const itemB = b.at(i);
    if (itemA.key !== itemB?.key || itemA.disabled !== itemB?.disabled) {
      return false;
    }
  }
...
function handleNumericSelection(
  sequence: string,
  itemsLength: number,
...
export function useSelectionList<T>({
  ...
  useEffect(() => {
    ...
    const activeItem = items.at(state.activeIndex);

    if (state.pendingHighlight && activeItem) {
      onHighlight?.(activeItem.value);
      needsClear = true;
    }

    if (state.pendingSelect && activeItem) {
  ...
  const handleKeypress = useCallback(
    (key: Key) => {
      ...
      if (isNumeric) {
        handleNumericSelection(sequence, itemsLength, dispatch, numberInputTimer, numberInputRef);
        return true;
      }
      return false;
    },
