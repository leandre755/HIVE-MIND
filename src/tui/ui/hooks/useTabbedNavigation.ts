...
function getNextTabState(state: TabbedNavigationState): TabbedNavigationState {
...
function getPrevTabState(state: TabbedNavigationState): TabbedNavigationState {
  const { tabCount, wrapAround, currentIndex } = state;
  if (tabCount === 0) return state;
...
function tabbedNavigationReducer(
  ...
  switch (action.type) {
    case 'NEXT_TAB':
      return getNextTabState(state);

    case 'PREV_TAB':
      return getPrevTabState(state);

    case 'SET_INDEX': {
      const { index } = action.payload;
      const { tabCount, currentIndex } = state;

      if (index === currentIndex || index < 0 || index >= tabCount) return state;
