...
export function useBackgroundTaskManager({
  ...
  const [activeBackgroundTaskPid, setActiveBackgroundTaskPid] = useState<number | null>(null);
  ...
  useEffect(() => {
    ...
    } else if (activeBackgroundTaskPid === null || !backgroundTasks.has(activeBackgroundTaskPid)) {
  ...
  }, [backgroundTasks, activeBackgroundTaskPid, backgroundTaskCount, isBackgroundTaskListOpen]);

  useEffect(() => {
    if (embeddedShellFocused) {
      const hasActiveForegroundShell = !!activePtyId;
      const hasVisibleBackgroundTask = isBackgroundTaskVisible && backgroundTasks.size > 0;
