...
import { ConsoleMessageItem, ConsoleLogPayload } from '../contexts/UIStateContext.js';
...
const handleConsoleLog = (payload: ConsoleLogPayload) => {
  const p = payload as ConsoleLogPayload & {
    message?: string;
    content?: string;
    type?: string;
    level?: string;
  };
  let content = p.message ?? p.content ?? '';
  ...
  const level = p.level ?? p.type ?? 'log';
  const msgType: 'log' | 'info' | 'warn' | 'error' =
    level === 'info' || level === 'warn' || level === 'error' ? level : 'log';
