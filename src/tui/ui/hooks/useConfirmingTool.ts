...
import { getConfirmingToolState, type ConfirmingToolState } from '../utils/confirmingTool.js';
...
export function useConfirmingTool(): ConfirmingToolState | null {
  ...
  // Agent responses and Slash commands.
  const { pendingHistoryItems } = useUIState();

  return useMemo(() => getConfirmingToolState(pendingHistoryItems), [pendingHistoryItems]);
}
