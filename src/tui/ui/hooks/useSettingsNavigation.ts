  ...
  return function navReducer(state: NavState, action: NavAction): NavState {
    ...
    switch (action.type) {
      case 'MOVE_UP': {
        ...
        return {
          activeItemKey: items.at(newIndex)?.key ?? null,
...
export function useSettingsNavigation({ items, maxItemsToShow }: UseSettingsNavigationProps) {
  ...
  const windowStart = useMemo(
    () => calculateSlidingWindow(state.windowStart, activeIndex, items.length, maxItemsToShow),
