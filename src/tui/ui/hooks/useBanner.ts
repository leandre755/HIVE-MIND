...
export function useBanner(bannerData: BannerData) {
  ...
  const currentBannerCount =
    new Map<string, number>(Object.entries(bannerCounts)).get(hashedText) ?? 0;

  const showBanner =
    activeText !== '' &&
    (currentBannerCount < DEFAULT_MAX_BANNER_SHOWN_COUNT || activeText.includes('Antigravity'));
  ...
  useEffect(() => {
    if (showBanner && activeText) {
      if (!sessionIncrementedBanners.has(activeText)) {
        ...
        const current = new Map<string, number>(Object.entries(allCounts)).get(hashedText) ?? 0;
