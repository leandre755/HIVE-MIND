...
class ThemeManager {
   ...
   */
  loadCustomThemes(customThemesSettings?: Record<string, unknown>): void {
    ...
    for (const [name, rawConfig] of Object.entries(customThemesSettings)) {
      const customThemeConfig = rawConfig as Partial<CustomTheme>;
  ...
  isThemeCompatible(activeTheme: Theme, terminalBackground: string | undefined): boolean {
    ...
    const themeType =
      activeTheme.type === 'custom'
        ? getThemeTypeFromBackgroundColor(
            resolveColor(activeTheme.colors.Background) || activeTheme.colors.Background,
          )
        : activeTheme.type;
