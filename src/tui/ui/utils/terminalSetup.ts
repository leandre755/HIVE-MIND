...
function getSupportedTerminalData(terminal: SupportedTerminal): TerminalData | null {
  return (Reflect.get(TERMINAL_DATA, terminal) as TerminalData | undefined) || null;
}
 ...
 */
function hasOurBinding(keybindings: unknown[], key: 'shift+enter' | 'ctrl+enter'): boolean {
...
async function configureVSCodeStyle(
  ...
  try {
    await safeMkdir(configDir, { recursive: true });

    let keybindings: unknown[] = [];
    try {
      const content = await safeReadFile(keybindingsFile, 'utf-8');
    ...
    await safeWriteFile(keybindingsFile, JSON.stringify(keybindings, null, 4));
    return {
      success: true,
...
export async function shouldPromptForTerminalSetup(): Promise<boolean> {
  ...
  try {
    ...
    debugLogger.debug(`Failed to read or parse keybindings, assuming prompt is needed: ${error}`);
    return true;
  }
}
