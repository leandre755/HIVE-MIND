...
function convertNamedCommands(text: string): string {
  return text.replace(
    /\\([A-Za-z]+)(?![A-Za-z])/g,
    (match, name: string) =>
      (Reflect.get(GREEK_LETTERS, name) as string | undefined) ??
      (Reflect.get(LATEX_COMMANDS, name) as string | undefined) ??
      match,
  );
}
...
function convertSubSuperScripts(text: string): string {
  ...
  let out = text.replace(/_\{([^{}]+)\}/g, (match, inner: string) => {
    ...
    if (chars.every((c) => Reflect.get(SUBSCRIPT_MAP, c) !== undefined)) {
      return chars.map((c) => Reflect.get(SUBSCRIPT_MAP, c) as string).join('');
    }
    return match;
  });
  out = out.replace(/\^\{([^{}]+)\}/g, (match, inner: string) => {
    const chars = charsOf(inner);
    if (chars.every((c) => Reflect.get(SUPERSCRIPT_MAP, c) !== undefined)) {
      return chars.map((c) => Reflect.get(SUPERSCRIPT_MAP, c) as string).join('');
    }
    return match;
  });
  ...
  out = out.replace(/([A-Za-z0-9)\]])_([A-Za-z0-9+\-=()])/g, (match, base: string, c: string) => {
    const sub = Reflect.get(SUBSCRIPT_MAP, c) as string | undefined;
    return sub ? `${base}${sub}` : match;
  });
  out = out.replace(/([A-Za-z0-9)\]])\^([A-Za-z0-9+\-=()])/g, (match, base: string, c: string) => {
    const sup = Reflect.get(SUPERSCRIPT_MAP, c) as string | undefined;
