...
export const isAtCommand = (query: string): boolean => {
  for (let i = 0; i < query.length - 1; i++) {
    ...
    if (char === '@' && prevChar !== '\\') {
      const nextChar = query.charAt(i + 1);
      if (nextChar && !' \t\n\r,;!?()[]{}.'.includes(nextChar)) {
        return true;
      }
    }
...
export const copyToClipboard = async (text: string, settings?: Settings): Promise<void> => {
  if (!text) return;
  ...
  if (shouldUseOsc52(tty, settings)) {
    ...
    let payload = osc;
    if (inTmux()) {
      payload = wrapForTmux(osc);
    } else if (inScreen()) {
      payload = wrapForScreen(osc);
    }
