...
function resolveX11EventName(b: number, isWheel: boolean, isMove: boolean): MouseEventName | null {
  if (isWheel) {
    const button = b & 3;
    if (button === 0) return 'scroll-up';
    if (button === 1) return 'scroll-down';
    return null;
  }
  if (isMove) return 'move';

  const button = b & 3;
  if (button === 3) return 'left-release';
  if (button === 0) return 'left-press';
  if (button === 1) return 'middle-press';
  if (button === 2) return 'right-press';
  return null;
}
...
export function parseX11MouseEvent(buffer: string): { event: MouseEvent; length: number } | null {
  ...
  const name = resolveX11EventName(b, isWheel, isMove);
