...
function renderHastNode(
  ...
  if (node.type === 'element') {
    ...
    for (let i = nodeClasses.length - 1; i >= 0; i--) {
      const cls = nodeClasses.at(i);
      const color = cls ? theme.getInkColor(cls) : undefined;
      if (color) {
        elementColor = color;
        break;
      }
...
export function colorizeCode({
  ...
  const useMaxSizedBox =
    (!settings.merged.ui.useAlternateBuffer || availableHeight !== undefined) && !returnLines;
  ...
  try {
    ...
    const fallbackLines = finalLines.map((line, index) => (
      <Box key={index} minHeight={1}>
        ...
        <Text color={disableColor ? undefined : activeTheme.colors.Gray}>{stripAnsi(line)}</Text>
      </Box>
    ));
