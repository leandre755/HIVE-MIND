...
export function stripUnsafeCharacters(str: string): string {
  ...
  const charCodesToKeep = (c: string): boolean => {
  ...
  const filtered = Array.from(strippedAnsi).filter(charCodesToKeep).join('');
  return stripVTControlCharacters(filtered);
}
...
function escapeArrayAnsiCtrlCodes<T>(arr: unknown[]): T {
  let newArr: unknown[] | null = null;

  for (let i = 0; i < arr.length; i++) {
    const value = arr.at(i);
    const escapedValue = escapeAnsiCtrlCodes(value);
    if (escapedValue !== value) {
      if (newArr === null) {
        newArr = [...arr];
      }
      Reflect.set(newArr, i, escapedValue);
    }
  }

  return (newArr !== null ? newArr : arr) as T;
}

function escapeObjectAnsiCtrlCodes<T>(obj: Record<string, unknown>): T {
  ...
  for (const key of keys) {
    ...
    if (escapedValue !== value) {
      ...
      Reflect.set(newObj, key, escapedValue);
    }
  }

  return (newObj !== null ? newObj : obj) as T;
}
...
export function escapeAnsiCtrlCodes<T>(obj: T): T {
  ...
  if (Array.isArray(obj)) {
    return escapeArrayAnsiCtrlCodes(obj);
  }

  return escapeObjectAnsiCtrlCodes(obj as Record<string, unknown>);
}
