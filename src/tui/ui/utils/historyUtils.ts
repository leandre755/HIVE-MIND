...
export function isToolExecuting(pendingHistoryItems: HistoryItemWithoutId[]): boolean {
  return pendingHistoryItems.some((item) => {
    if (item && item.type === 'tool_group') {
      const toolGroup = item as unknown as HistoryItemToolGroup;
      return toolGroup.tools?.some((tool) => CoreToolCallStatus.Executing === tool.status) ?? false;
    }
    return false;
  });
}

export function isToolAwaitingConfirmation(pendingHistoryItems: HistoryItemWithoutId[]): boolean {
  return pendingHistoryItems
    .filter((item): item is HistoryItemToolGroup => item.type === 'tool_group')
    .some(
      (item) =>
        item.tools?.some((tool) => CoreToolCallStatus.AwaitingApproval === tool.status) ?? false,
    );
}

export function getAllToolCalls(historyItems: HistoryItemWithoutId[]): IndividualToolCallDisplay[] {
  return historyItems
    .filter((item): item is HistoryItemToolGroup => item.type === 'tool_group')
