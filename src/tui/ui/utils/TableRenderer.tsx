...
export const TableRenderer: React.FC<TableRendererProps> = ({ headers, rows, terminalWidth }) => {
  ...
  const { wrappedHeaders, wrappedRows, adjustedWidths } = useMemo(() => {
    ...
    const constraints = Array.from({ length: numColumns }).map((_, colIndex) => {
      const headerStyledLine = styledHeaders.at(colIndex) || StyledLine.empty(0);
      let { contentWidth: maxContentWidth, maxWordWidth } = calculateWidths(headerStyledLine);

      styledRows.forEach((row) => {
        const cellStyledLine = row.at(colIndex) || StyledLine.empty(0);
    ...
    const wrapAndProcessRow = (row: StyledLine[]) => {
      ...
      for (let colIndex = 0; colIndex < numColumns; colIndex++) {
        const cellStyledLine = row.at(colIndex) || StyledLine.empty(0);
        const allocatedWidth = finalContentWidths.at(colIndex) ?? 0;
        ...
        Reflect.set(
          actualColumnWidths,
          colIndex,
          Math.max(actualColumnWidths.at(colIndex) ?? 0, maxLineWidth),
        );
    ...
    return {
      wrappedHeaders: adjustedWrappedHeaders,
      wrappedRows: adjustedWrappedRows,
      adjustedWidths: finalWidths,
    };
  ...
  const renderBorder = (type: 'top' | 'middle' | 'bottom'): React.ReactNode => {
    ...
    const char = Reflect.get(chars, type) as {
      left: string;
      middle: string;
      right: string;
      horizontal: string;
    };
    const borderParts = adjustedWidths.map((w) => char.horizontal.repeat(Math.max(0, w || 0)));
  ...
  const renderVisualRow = (cells: ProcessedLine[], isHeader = false): React.ReactNode => {
    const renderedCells = cells.map((cell, index) => {
      const width = adjustedWidths.at(index) ?? 0;
  ...
  const renderDataRow = (
    ...
    for (let i = 0; i < maxHeight; i++) {
      const visualRowCells = wrappedCells.map((lines) => lines.at(i) || { text: '', width: 0 });
      visualRows.push(
