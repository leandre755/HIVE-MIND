...
import { safeExistsSync, safeStatSync } from '../../../utils/safeFs.js';
...
export async function getDirectorySuggestions(partialPath: string): Promise<string[]> {
  try {
    ...
    if (!safeExistsSync(searchDir) || !safeStatSync(searchDir)?.isDirectory()) {
      return [];
    }
...
export function batchAddDirectories(
  ...
  if (!workspaceContext.addDirectories) {
    return { added: [], errors: ['Workspace context does not support adding directories.'] };
  }
