...
export function calculateToolContentMaxLines(options: {
  ...
  if (maxLinesLimit !== undefined) {
    contentHeight =
      contentHeight !== undefined ? Math.min(contentHeight, maxLinesLimit) : maxLinesLimit;
  }
...
export function calculateShellMaxLines(options: {
  ...
  if (availableTerminalHeight === undefined) {
    return isAlternateBuffer ? ACTIVE_SHELL_MAX_LINES - SHELL_CONTENT_OVERHEAD : undefined;
  }
