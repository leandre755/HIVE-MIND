...
export function deriveItemsFromLegacySettings(settings: MergedSettings): string[] {
  const defaults = ['workspace', 'git-branch', 'sandbox', 'model-name', 'quota'];
  ...
  if (!settings.ui.footer?.hideContextPercentage && !items.includes('context-used')) {
...
export function resolveFooterState(settings: MergedSettings): {
  ...
  const source = (settings.ui?.footer?.items ?? deriveItemsFromLegacySettings(settings)).filter(
    (id: string) => filteredValidIds.has(id),
  );

  const others = DEFAULT_ORDER.filter((id) => !source.includes(id) && filteredValidIds.has(id));
