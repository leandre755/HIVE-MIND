import { safeExistsSync, safeReadFileSync, safeMkdirSync } from '../../utils/safeFs.js';
...
export interface AdminControlsSettings {
  ...
  mcpSetting?: unknown;
  cliFeatureSetting?: unknown;
}

export function createCache<K, V>(_options?: {
  ttlMs?: number;
  storage?: string;
  defaultTtl?: number;
}): {
...
export function getMergeStrategyForPath(settingsPath: string[]): MergeStrategy | undefined {
  let current: SettingDefinition | undefined = undefined;
  let currentSchema: SettingsSchema | undefined = getSettingsSchema();
  let parent: SettingDefinition | undefined = undefined;

  for (const key of settingsPath) {
    const currentSchemaMap: Map<string, SettingDefinition> = new Map(
      Object.entries(currentSchema ?? {}),
    );
    const currentDef: SettingDefinition | undefined = currentSchemaMap.get(key);
    if (!currentSchema || !currentDef) {
...
export const DEFAULT_EXCLUDED_ENV_VARS = [
  'DEBUG',
  'DEBUG_MODE',
  'HIVE_MIND_IDE_SERVER_STDIO_COMMAND',
  'HIVE_MIND_IDE_SERVER_STDIO_ARGS',
];

const AUTH_ENV_VAR_WHITELIST = ['GEMINI_API_KEY', 'GOOGLE_API_KEY', 'GOOGLE_CLOUD_LOCATION'];
...
// Removed Gemini-CLI specific system paths
...
export type LoadableSettingScope =
  SettingScope.User | SettingScope.Workspace | SettingScope.System | SettingScope.SystemDefaults;
...
function setNestedProperty(obj: Record<string, unknown>, settingsPath: string, value: unknown) {
  ...
  for (const key of keys) {
    ...
    if (next === undefined) {
      ...
      Reflect.set(current, key, nextObj);
      current = nextObj;
...
export function getDefaultsFromSchema(schema: SettingsSchema = getSettingsSchema()): Settings {
  ...
  const schemaMap = new Map<string, SettingDefinition>(Object.entries(schema));
  for (const [key, definition] of schemaMap) {
    if (definition.properties) {
      defaults.set(key, getDefaultsFromSchema(definition.properties));
    } else if (definition.default !== undefined) {
      defaults.set(key, definition.default);
    }
  }
  return Object.fromEntries(defaults) as Settings;
}
...
export function mergeSettings(
  ...
  return customDeepMerge(
    ...
    schemaDefaults as unknown as MergeableObject,
    systemDefaults as unknown as MergeableObject,
    user as unknown as MergeableObject,
    safeWorkspace as unknown as MergeableObject,
    system as unknown as MergeableObject,
...
export function createTestMergedSettings(overrides: Partial<Settings> = {}): MergedSettings {
  return customDeepMerge(
    getMergeStrategyForPath,
    getDefaultsFromSchema() as unknown as MergeableObject,
    overrides as unknown as MergeableObject,
  ) as unknown as MergedSettings;
}
...
export class LoadedSettings {
  ...
  private computeMergedSettings(forceTrusted = false): MergedSettings {
    ...
    if (adminSettingSchema?.properties) {
      ...
      merged.admin = customDeepMerge(
        ...
        adminDefaults as unknown as MergeableObject,
        (this._remoteAdminSettings?.admin ?? {}) as unknown as MergeableObject,
  ...
  setRemoteAdminSettings(remoteSettings: AdminControlsSettings): void {
    ...
    const mcp = mcpSetting as
      | {
          mcpEnabled?: boolean;
          mcpConfig?: { mcpServers?: unknown };
          requiredMcpConfig?: unknown;
        }
      | undefined;
    const cliFeature = cliFeatureSetting as
      | {
    ...
    admin.mcp = {
      enabled: mcp?.mcpEnabled,
      config: mcp?.mcpConfig?.mcpServers,
      requiredConfig: mcp?.requiredMcpConfig,
    };
    admin.extensions = {
      enabled: cliFeature?.extensionsSetting?.extensionsEnabled,
    };
    admin.skills = {
      enabled: cliFeature?.unmanagedCapabilitiesEnabled,
    };
  ...
  getConsolidatedAllowedMcpServers(): string[] | undefined {
    ...
    return definedAllowlists.reduce((acc, current) => {
      const normalizedCurrent = new Set(current.map((item) => item.toLowerCase().trim()));
      return acc.filter((item) => normalizedCurrent.has(item.toLowerCase().trim()));
    });
  }
}

function checkHomeEnvFallback(isTrusted: boolean): string | null {
  ...
  if (isTrusted) {
    ...
    if (safeExistsSync(homeHiveEnvPath)) {
      return homeHiveEnvPath;
    }
  }
  const homeEnvPath = path.join(homedir(), '.env');
  if (safeExistsSync(homeEnvPath)) {
    return homeEnvPath;
  }
  return null;
}

function findEnvFile(startDir: string, isTrusted: boolean, ignoreLocalEnv: boolean): string | null {
  ...
  for (let depth = 0; depth < MAX_DEPTH; depth++) {
    ...
    if (parentDir === currentDir || !parentDir) {
      return checkHomeEnvFallback(isTrusted);
    }
...
// Cloud shell specific code removed.

function applyEnvFile(envFilePath: string, settings: Settings, isTrusted: boolean): void {
  try {
    const envFileContent = safeReadFileSync(envFilePath);
    ...
    for (const [key, rawValue] of Object.entries(parsedEnv)) {
      ...
      if (!processEnvMap.has(key)) {
        processEnvMap.set(key, value);
      }
    }
    for (const [key, value] of processEnvMap) {
      if (value !== undefined) {
...
export function loadEnvironment(
  ...
  // Cloud shell env var block removed

  if (envFilePath) {
    // Manually parse and load environment variables to handle exclusions correctly.
    // This avoids modifying environment variables that were already set from the shell.
    applyEnvFile(envFilePath, settings, isTrusted);
  }
}
...
function _doLoadSettings(workspaceDir: string): LoadedSettings {
  ...
  const systemDefaultsOriginalSettings = structuredClone(systemDefaultsResult.rawSettings);
  ...
  const initialTrustCheckSettings = customDeepMerge(
    getMergeStrategyForPath,
    getDefaultsFromSchema() as unknown as MergeableObject,
    systemDefaultSettings as unknown as MergeableObject,
    userSettings as unknown as MergeableObject,
    systemSettings as unknown as MergeableObject,
  );
  const isTrusted =
    isWorkspaceTrusted(initialTrustCheckSettings as Settings, workspaceDir).isTrusted ?? false;
...
function migrateBoolean(
  ...
  const oldValue = settingsMap.get(oldKey);
  const newValue = settingsMap.get(newKey);
  ...
  if (typeof oldValue === 'boolean') {
    ...
    if (typeof newValue === 'boolean') {
      ...
      if (removeDeprecated) {
        settingsMap.delete(oldKey);
        modified = true;
      }
    } else {
      // Only old exists, migrate to new (inverted)
      settingsMap.set(newKey, !oldValue);
      if (removeDeprecated) {
        settingsMap.delete(oldKey);
      }
      modified = true;
  ...
  Object.assign(settings, Object.fromEntries(settingsMap));
  return modified;
}
...
function migrateContextSettings(
  settings: Settings,
  scope: LoadableSettingScope,
  loadedSettings: LoadedSettings,
  settingsFile: SettingsFile,
...
function migrateToolsSettings(
  ...
  if (toolsSettings['approvalMode'] !== undefined) {
    ...
    const currentGeneralSettings = (settings.general as Record<string, unknown> | undefined) || {};
...
export function migrateDeprecatedSettings(
  ...
  const processScope = (scope: LoadableSettingScope) => {
    ...
    const generalModified = migrateGeneralSettings(
      settings,
      scope,
      loadedSettings,
      settingsFile,
    ...
    // Migrate ui settings
    const uiModified = migrateUiSettings(
      settings,
      scope,
      loadedSettings,
      settingsFile,
    ...
    const contextModified = migrateContextSettings(
      settings,
      scope,
      loadedSettings,
      settingsFile,
    ...
    const toolsModified = migrateToolsSettings(
      settings,
      scope,
      loadedSettings,
      settingsFile,
    ...
    if (generalModified || uiModified || contextModified || toolsModified || experimentalModified) {
  ...
  if (systemWarnings.size > 0) {
    for (const [scope, flags] of systemWarnings) {
      const scopeName =
        scope === SettingScope.SystemDefaults ? 'system default' : scope.toLowerCase();
...
function migrateExperimentalSettings(
  ...
  if (experimentalSettings) {
    ...
    const agentsMap = new Map<string, unknown>(Object.entries(agentsSettings));
    const agentsOverrides = {
      ...(agentsMap.get('overrides') as Record<string, unknown> | undefined),
    };
    let modified = false;
    ...
    const migrateExperimental = <T = Record<string, unknown>>(
      ...
      const experimentalMap = new Map<string, unknown>(Object.entries(experimentalSettings));
      const old = experimentalMap.get(oldKey);
      if (old !== undefined) {
    ...
    migrateExperimental<boolean>('plan', (planValue) => {
      const generalSettings = (settings.general as Record<string, unknown> | undefined) || {};
      const newGeneral = { ...generalSettings };
      const planSettings = (newGeneral['plan'] as Record<string, unknown> | undefined) || {};
