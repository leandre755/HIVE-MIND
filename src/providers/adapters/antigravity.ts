...
async function getTokens(endpoint: string): Promise<{ accessToken: string; projectId: string }> {
  ...
  if (!accessToken) {
    throw new Error('Antigravity access token unavailable after refresh');
  }
...
function parseToolArguments(rawArguments: unknown): Record<string, unknown> {
  ...
  try {
    ...
    console.warn('[Antigravity] Arguments de tool_call JSON invalides, objet vide transmis.');
    return {};
  }
}
...
function parseAntigravityResponse(data: CodeAssistResponse): AdapterChatResult {
...
export async function chat(
  ...
  // Télémétrie Clearcut émise en tâche de fond : son échec ne doit pas
  // interrompre l'appel au modèle, mais il est tracé et non avalé.
  ClearcutSimulator.trackStartSession().catch((err: unknown) => {
    console.warn('[Antigravity] Télémétrie trackStartSession en échec:', err);
  });
  ...
  const wrappedBody: CodeAssistRequestWrapper = {
    project: projectId,
    model: requireModel(model, 'Antigravity Adapter'),
    request: payload,
    requestType: 'chat',
  ...
  ClearcutSimulator.trackNewPrompt(requestId).catch((err: unknown) => {
    console.warn('[Antigravity] Télémétrie trackNewPrompt en échec:', err);
  });
